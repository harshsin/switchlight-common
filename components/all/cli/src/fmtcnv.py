...
def convert_mac_in_byte_array_to_hex_string(i, data=None):
    if i == "*" or i == "": 
        return "*"
    return ":".join([format(x, "02x") for x in i])
...
def timestamp_ns_to_local_timestr(i, data=None):
    #the input is the elapsed time in nanosecond from install to current
    seconds      = i / 1000000000
    seconds = time.mktime(time.localtime())-seconds
    seconds=seconds -(seconds % 10) #ease the inaccuracy
    timestr = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(seconds))
    return timestr
    ...
    for a in action_list:
        ...
        if decoded_action == "OUTPUT":
            ...
            if 'vendor' in a:
                ...
                #    if 'subtype' in a and a['subtype']==TTL_DECREMENT_SUBTYPE:
                    decoded_action="nicira_vendor"
                if a['vendor']==BSN_VENDOR_ID:
                #    if 'subtype' in a and a['subtype']==SET_TUNNEL_DST_SUBTYPE:
                    decoded_action="bsn_vendor"
...
def formatter_to_alias_update(formatter, update):
    ...
    if name in ['print_host_and_alias',
                'print_host_and_alias_mac_as_long',
    ...
    if name in ['print_host_attachment_point',
                'print_host_and_alias_mac_as_long',
                'print_host_and_alias_mac_as_long_str',
