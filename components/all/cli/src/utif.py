...
def cidr_to_prefix(addr):
    """
    Converts the specified address from IP/mask to its routing prefix.
    E.g. '10.192.75.4/18' => '10.192.64.0/18'
    """
    split = addr.split('/')
    mask = (0xffffffff << (32 - int(split[1]))) & 0xffffffff
    return inet_ntoa(inet_aton(split[0]) & mask) + '/' + split[1]
