...
def validate_ip_address(typedef, value):
...
DPID_RE = re.compile(r'^[A-Fa-f\d]{1,16}$')

def validate_dpid(typedef, value):
    # strip out colons
    s = value.translate(None, ':')
    if not DPID_RE.match(s):
...
def validate_identifier(typedef, value, reserved = None):
    ...
    if not re.match(r'^[a-zA-Z0-9_-]+$', str(value)):
        ...
        command._raise_argument_validation_exception(typedef, value, msg)
    if value in bigsh.reserved_words:
        msg = 'reserved word "%s" in "%s"' % (value, ', '.join(bigsh.reserved_words))
        command._raise_argument_validation_exception(typedef, value, msg)
...
def init_validations(bs):
    ...
    command.add_validation('validate-ip-address', validate_ip_address)
    ...
    command.add_validation('validate-dpid', validate_dpid)
