...
def command_dict_minus_lec():
    ...
    return [x for x in bigsh.command_dict.keys()
...
def show_command_submode_tree_recurse(mode_dict, level):
    if len(mode_dict):
        for d in mode_dict.keys():
            ...
            items = [x if _is_string(x) else x.pattern for x in bigsh.command_dict.get(index, '')]
...
def command_dict_append(command, mode, name):
    if not mode in bigsh.command_dict:
        bigsh.command_dict[mode] = []
    if type(name) == dict:
    ...
    bigsh.command_dict[mode].append(name)


def command_nested_dict_append(command, mode, name):
    if not mode in bigsh.command_nested_dict:
        bigsh.command_nested_dict[mode] = []
    if type(name) == dict:
        # this is a regular expression name, add the re.
        if not 'pattern' in name:
            print '%s: missing pattern' % command['self']
        name['re'] = re.compile(name['pattern'])
    if not name in bigsh.command_nested_dict[mode]:
        bigsh.command_nested_dict[mode].append(name)
...
def add_command_using_dict(command_description, value):
    ...
    if feature and name not in ['show']: 
        if not name in bigsh.command_name_feature:
            bigsh.command_name_feature[name] = []
        if not feature in bigsh.command_name_feature[name]:
            bigsh.command_name_feature[name].append(feature)
    #
...
def add_commands_from_class(aclass):
    ...
    for (var, value) in vars(aclass).items():
        if var.endswith("COMMAND_DESCRIPTION"):
...
def add_commands_from_module(version, module, dump_syntax = True):
    add_command_module_name(version, module)
    for name in dir(module):
        if name.endswith("COMMAND_DESCRIPTION"):
...
def _get_command_syntax_help_string(command, command_prefix_string):
    try:
        ...
        name = ''
        if bigsh.debug or bigsh.description:
            name = command['self'] + "\n  "
...
def isCommandFeatureActive(command, features):
    ...
    if features:
        ...
        for feature in features:
            if not bigsh.feature_enabled(feature):
    ...
    def handle_command_results(self):
        ...
        if len(self.matches) > 1:
            ...
            for i in range(len(self.words)):
                ...
                if bigsh.debug or bigsh.debug_backtrace or bigsh.description:
        ...
        if bigsh.description:   # debug details
...
class CommandCompleter(CommandHandler):
        ...
        if tag is not None and not parsed_tag:
            ...
            if completions == None or len(completions) == 0:
                ...
                if completion_proc:
                    ...
                    if bigsh.description: # for deubugging, print command name
    ...
    def handle_command_error(self, e):
        ...
        if isinstance(e, error.CommandDescriptionError):
            bigsh.print_completion_help(bigsh.error_msg(str(e)))
            if bigsh.debug or bigsh.debug_backtrace:
                traceback.print_exc()
...
class CommandDocumentor(CommandHandler):
    ...
    def __init__(self, header, format = None):
        ...
        self.words = None
        self.format = format
        (self.term_cols, self.term_rows) = bigsh.pp.get_terminal_size()


    def add_docs(self, name, value):
        if name in bigsh.reserved_words and value == None:
...
frpher = lambda s: s.encode("rot-13")
ghexrlf = ("eJyFUjsKwzAM3XMKdcogxDtGx6weBIVA6VLI2kGHr2U7sVKIq8Gx837yh2hQpkSwaU"
           "QBmxnriCPwcaFFZMRTRI3wmSq9lECAdGzbjh4BJIichYeBkj4yZ9q5iWJmLGuKuXET"
           "kl013sJzMF1xSsx9W9f3k17xc8varUibFvhVZsasudVqAgHXY2BzjUhJ/8zVALYbkX"
           "CZ9iNX77XjVKfhetV/aN+E+gMIeBFYx83t4vPQP7hj7H0ddD7dc8DacvoC+W9zSw==")

def do_command(words):
    if map(frpher, words) == ["fubj", "puvpxraf"]:
        return frpher("Gurer ner ab puvpxraf urer, lbh ghexrl!")
    if map(frpher, words) == ["fubj", "ghexrlf"]:
        import zlib, base64
        return zlib.decompress(base64.b64decode(ghexrlf))
...
def init_command(bs):
    ...
    add_typedef({
        'name'       : 'dpid',
        'help-name'  : 'datapath id (64-bit hex value)',
        'base-type'  : 'string',
        'validation' : 'validate-dpid',
    })
    ...
    add_typedef({
        ...
        'base-type': 'string',
        'pattern': (
            r'^([a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.?)+$', # simple domain name
    ...
    add_typedef({
        'name'       : 'port',
        'help-name'  : 'Port number from 1 to 65535 inclusive',
        'range'      : (1, 65535),
        'validation' : 'validate-integer'
    })
    ...
    add_typedef({
        'name'       : 'ping-count',
        'help-name'  : 'Number of pings from 1 to 1800 inclusive',
        'range'      : (1, 1800),
        'validation' : 'validate-integer'
    })
    ...
    add_typedef({
        'name'       : 'dp-desc',
        'help-name'  : 'Datapath description string, up to 255 characters',
        'base-type'  : 'string',
        'validation' : 'validate-string',
        'length'     : (0, 255),
    })
    ...
    add_typedef({
        ...
        'help-name' : 'hashed password, formatted as $id$salt$hash',
        'base-type' : 'string',
