...
modules:
	$(MAKE) -C $(SDK)/systems/switchlight/amd64/$(SDK_CONFIG) SDK=$(SDK) \
		SWITCHLIGHT_LINUX_KERNEL=$(KERNEL_HEADERS) BLDCONFIG=$(BLDCONFIG) modules
	cp $(SDK)/build/$(BLDCONFIG)/switchlight/amd64/$(SDK_CONFIG)/linux-kernel-bde.ko $(MODULE_DIR)/linux-kernel-bde-$(SDK_KMOD_VERSION).ko
	cp $(SDK)/build/$(BLDCONFIG)/switchlight/amd64/$(SDK_CONFIG)/linux-user-bde.ko $(MODULE_DIR)/linux-user-bde-$(SDK_KMOD_VERSION).ko
