                 ...
                 int service, unsigned char* data)
{
    ...
    if( sofc_message_type_recv(sc->sofc, sc->id, OF_BSN_SHELL_OUTPUT, msg) >= 0) {
        of_octets_t data;
        of_bsn_shell_output_t* output = (of_bsn_shell_output_t*) of_object_new_from_message(msg, of_message_length_get(msg));
    ...
    if( sofc_message_type_recv(sc->sofc, sc->id, OF_BSN_SHELL_STATUS, msg) >= 0) {
        uint32_t rv;
        of_bsn_shell_status_t* status = (of_bsn_shell_status_t*) of_object_new_from_message(msg, of_message_length_get(msg));
