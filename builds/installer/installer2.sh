...
installer_platform_loader() {
    ...
    if [ "${platform_loader}" ]; then
        ...
        local loader="${installer_platform_dir}/switchlight.${installer_platform}.loader"
    fi
...
#
# IF the installer filename contains 'ztn' then we
# will install in ZTN mode by default. 
# 
# Otherwise, we will install in standalone mode.
#
# The only difference is the name of SWI file
# in the boot-config file. 
#
# This allows the same installer image to be 
# served with different names based on the
# intended operating mode.
#
installer_filename="$0"
without_ztn=`echo ${installer_filename} | sed 's/ztn//'`
if [ "${installer_filename}" = "${without_ztn}" ]; then
    # Not in ZTN mode.
    :
else
    # ZTN mode.
    installer_mode_ztn=1
fi
