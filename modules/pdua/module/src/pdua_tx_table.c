...
indigo_error_t
pdua_tx_table_init(void)
{
    if (init_done__) {
        AIM_LOG_INFO("%s: pdu-tx table already initialized", __FUNCTION__);
        return INDIGO_ERROR_NONE;
    }

    indigo_core_gentable_register("pdu-tx", &table_ops__, NULL, 1, 4,
                                  &gentable__);
    init_done__ = true;
    return INDIGO_ERROR_NONE;
}
            ...
            void **entry_priv)
{
    ...
    rv = parse_value__(value, &port_no, &interval, &data);
    if (rv != INDIGO_ERROR_NONE) {
    ...
    rv = pdua_port_tx_enable(port, &data, interval);
    if (rv != INDIGO_ERROR_NONE) {
        AIM_LOG_ERROR("%s: error enabling pdua tx on OF port %u",
                      __FUNCTION__, port_no);
        return rv;
    }
               ...
               of_list_bsn_tlv_t *value)
{
    ...
    rv = pdua_port_tx_disable(port);
    if (rv != INDIGO_ERROR_NONE) {
        AIM_LOG_ERROR("%s: error disabling pdua tx on OF port %u",
                      __FUNCTION__, port_no);
        return rv;
    }
