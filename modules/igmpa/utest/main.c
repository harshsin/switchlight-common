...
/* bad igmp checksum */
/* igmp_packet(eth_src='0:1:2:3:4:5',vlan_vid=10,
 * ip_src='1.1.1.1',ip_dst='224.0.0.1',
 * igmp_max_response_time=20) with IGMP checksum zeroed */
uint8_t igmp_bad_checksum[] = {
    ...
    0x00, 0x00, 0x01, 0x02, 0xd7, 0xdc, 0x01, 0x01,
...
/* igmp_packet(eth_src='0:1:2:3:4:5',vlan_vid=10,
 * ip_src='1.1.1.1',ip_dst='224.0.0.1',
 * igmp_max_response_time=20,ip_flags=1) */
uint8_t igmp_first_frag[] = {
    0x01, 0x00, 0x5e, 0x00, 0x00, 0x01, 0x00, 0x01,
    0x02, 0x03, 0x04, 0x05, 0x81, 0x00, 0x00, 0x0a,
    0x08, 0x00, 0x45, 0x00, 0x00, 0x1c, 0x00, 0x01,
    0x20, 0x00, 0x01, 0x02, 0xb7, 0xdc, 0x01, 0x01,
    0x01, 0x01, 0xe0, 0x00, 0x00, 0x01, 0x11, 0x14,
...
/* igmp_packet(eth_src='0:1:2:3:4:5',vlan_vid=10,
 * ip_src='1.1.1.1',ip_dst='224.0.0.1',
 * igmp_max_response_time=20,ip_frag=0x123) */
uint8_t igmp_last_frag[] = {
    0x01, 0x00, 0x5e, 0x00, 0x00, 0x01, 0x00, 0x01,
    0x02, 0x03, 0x04, 0x05, 0x81, 0x00, 0x00, 0x0a,
    0x08, 0x00, 0x45, 0x00, 0x00, 0x1c, 0x00, 0x01,
    0x01, 0x23, 0x01, 0x02, 0xd6, 0xb9, 0x01, 0x01,
    0x01, 0x01, 0xe0, 0x00, 0x00, 0x01, 0x11, 0x14,
    0xee, 0xeb, 0x00, 0x00, 0x00, 0x00,
};
...
/* bad pim checksum */
/* pim_hello(eth_src='0:1:2:3:4:5',vlan_vid=10,ip_src='2.2.2.2')
 * with checksum zeroed */
uint8_t pim_bad_checksum[] = {
    0x01, 0x00, 0x5e, 0x00, 0x00, 0x0d, 0x00, 0x01,
    0x02, 0x03, 0x04, 0x05, 0x81, 0x00, 0x00, 0x0a,
    0x08, 0x00, 0x45, 0x00, 0x00, 0x18, 0x00, 0x01,
    0x00, 0x00, 0x01, 0x67, 0xd5, 0x6d, 0x02, 0x02,
    0x02, 0x02, 0xe0, 0x00, 0x00, 0x0d, 0x20, 0x00,
    0x00, 0x00,
};
...
/* pim_hello(eth_src='0:1:2:3:4:5',vlan_vid=10,ip_src='2.2.2.2',
 * ip_flags=1) */
uint8_t pim_first_frag[] = {
    0x01, 0x00, 0x5e, 0x00, 0x00, 0x0d, 0x00, 0x01,
    0x02, 0x03, 0x04, 0x05, 0x81, 0x00, 0x00, 0x0a,
    0x08, 0x00, 0x45, 0x00, 0x00, 0x18, 0x00, 0x01,
    0x20, 0x00, 0x01, 0x67, 0xb5, 0x6d, 0x02, 0x02,
    0x02, 0x02, 0xe0, 0x00, 0x00, 0x0d, 0x20, 0x00,
    0xdf, 0xff,
};
...
/* pim_hello(eth_src='0:1:2:3:4:5',vlan_vid=10,ip_src='2.2.2.2',
 * ip_frag=0x123) */
uint8_t pim_last_frag[] = {
    0x01, 0x00, 0x5e, 0x00, 0x00, 0x0d, 0x00, 0x01,
    0x02, 0x03, 0x04, 0x05, 0x81, 0x00, 0x00, 0x0a,
    0x08, 0x00, 0x45, 0x00, 0x00, 0x18, 0x00, 0x01,
    0x01, 0x23, 0x01, 0x67, 0xd4, 0x4a, 0x02, 0x02,
    0x02, 0x02, 0xe0, 0x00, 0x00, 0x0d, 0x20, 0x00,
    0xdf, 0xff,
};
...
int aim_main(int argc, char* argv[])
{
    ...
    /* first frag, should forward to controller */
    printf("*** igmp first frag\n");
    rc = send_packet_in(10, sizeof(igmp_first_frag), igmp_first_frag);
    AIM_ASSERT(rc == INDIGO_CORE_LISTENER_RESULT_PASS);

    /* last frag, should forward to controller */
    printf("*** igmp last frag\n");
    rc = send_packet_in(10, sizeof(igmp_last_frag), igmp_last_frag);
    AIM_ASSERT(rc == INDIGO_CORE_LISTENER_RESULT_PASS);
    ...
    /* bad checksum, should forward to controller */
    printf("*** pim bad checksum\n");
    rc = send_packet_in(10, sizeof(pim_bad_checksum), pim_bad_checksum);
    AIM_ASSERT(rc == INDIGO_CORE_LISTENER_RESULT_PASS);

    /* first frag, should forward to controller */
    printf("*** pim first frag\n");
    rc = send_packet_in(10, sizeof(pim_first_frag), pim_first_frag);
    AIM_ASSERT(rc == INDIGO_CORE_LISTENER_RESULT_PASS);

    /* last frag, should forward to controller */
    printf("*** pim last frag\n");
    rc = send_packet_in(10, sizeof(pim_last_frag), pim_last_frag);
    AIM_ASSERT(rc == INDIGO_CORE_LISTENER_RESULT_PASS);
